...
describe("POST /api/auth/login", () => {
  ...
  it("returns 200 with admin payload on valid credentials", async () => {
    await seedAdmin();
...
describe("POST /api/auth/logout", () => {
  ...
  it("clears the token cookie in the response", async () => {
    ...
    const cookies = res.headers["set-cookie"] || [];
    const tokenCookie = cookies.find((c) => c.startsWith("token="));
    // clearCookie sets an expired date so the browser deletes it
...
describe("GET /api/auth/me", () => {
  ...
  it("returns the admin document without the password field", async () => {
    ...
    expect(res.body.name).toBe(adminCredentials.name);
